...
def add_filters_from_file(forum,filename):
    with open(filename) as file:
        for line in file:
            word_to_change = line.split(",")[0].strip()
...
async def track_activity(forum, filename="activity.log", sep="|"):
    while True:
        async with browser_lock:
            ...
            while True:
                try:
                    anon = False
                    display_name = driver.find_element(By.CSS_SELECTOR, f'.innertable tr:nth-child({i}) > .innercontent:nth-child(1)').text
                    if display_name.startswith("*"):
                        anon = True
                        display_name = display_name.replace("*", "")
                    last_action = driver.find_element(By.CSS_SELECTOR, f'.innertable tr:nth-child({i}) > .innercontent:nth-child(2)').text
                    last_activity = parse_time(driver.find_element(By.CSS_SELECTOR, f'tr:nth-child({i}) > .innercontent:nth-child(3)').text).strftime("%Y-%m-%d %H:%M:%S")
                    ip_address = driver.find_element(By.CSS_SELECTOR, f'tr:nth-child({i}) > .innercontent:nth-child(4)').text
                    arr.append([current_time, display_name, last_action, last_activity, ip_address])
                    i += 1
                    with open(filename, "a", encoding="cp932", errors="replace") as file:
                        file.write(f"{current_time}{sep}{display_name}{sep}{last_action}{sep}{last_activity}{sep}{ip_address}{sep}{anon}\n")
                except NoSuchElementException:
                    break
            arr.clear()
            print(f"[{current_time}] [ACTIVITY]: Activity tracker updated.")
        #await asyncio.sleep(1795)
        await asyncio.sleep(30)

async def get_confessional_topics(forum,mask=14,time_limit=30):
    async with browser_lock:
        driver.get(forum.url + f"/activetopics.asp?viewAs={mask}")


def start(subdomain):
    forum = Forum(subdomain=subdomain)
    activity_thread = threading.Thread(target=track_activity(forum))
    activity_thread.daemon = True
    activity_thread.start()

def main():
    forum = Forum(subdomain="void")
