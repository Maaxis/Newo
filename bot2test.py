...
class TribeCreator(discord.ui.Select):
	def __init__(self, tribe_name=None, user_id=None, tribe_role=None, tribe_channel=None):
	...
	async def callback(self, interaction: discord.Interaction):
		...
		for member in this_guild.members:
			if any(role in member.roles for role in tribe_member_roles):
				await member.add_roles(self.tribe_role)
		self.disabled = True
		await interaction.delete_original_response()
...
@bot.tree.command()
async def player(interaction: discord.Interaction, name: str, role: discord.Role = None, confessional_channel: discord.TextChannel = None,
                 tribe: discord.Role = None, status: Literal["Contestant","Jury","Prejury"] = None,
                 placement: int = None):
	this_guild = interaction.guild
	if tribe:
		tribe = db.get_snowflake_from_tribe_role(tribe.id)
	print(tribe)
	contestant = 0
	jury = 0
	prejury = 0
	...
	snowflake = ""
	if role:
		snowflake = db.get_snowflake_from_player_role(role.id)
	else:
		role = await this_guild.create_role(name=name)
	if not snowflake:
		snowflake = db.get_snowflake_from_player_name(name)
	print(snowflake)
	if not snowflake: # If player does not exist
		if confessional_channel:
			confessional_channel = confessional_channel.id
		confessional = db.add_confessional(player_snowflake=None, player_name=name, discord_channel_id=confessional_channel) # Create confessional without player snowflake
		player = db.add_player(name, role.id, confessional, tribe, contestant, jury, prejury, placement) # Create player
